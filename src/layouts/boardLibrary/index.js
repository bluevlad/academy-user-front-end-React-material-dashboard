...
// API
import { getBoardLibraryList } from "api/boardLibrary";
...
function BoardLibrary() {
  ...
  const loadBoardData = async () => {
    ...
    try {
      ...
      const data = await getBoardLibraryList(params);
      ...
      console.error("Failed to load board library data:", error);
      setBoardList([]);
    } finally {
      setLoading(false);
    }
  ...
  const columns = [
    ...
    { Header: "첨부", accessor: "fileIcon", width: "10%", align: "center" }, // Library typically has files
  ...
  const rows = boardList.map((board) => ({
    ...
    fileIcon: (
      <MDTypography variant="caption" color="text" fontWeight="regular">
        {board.attachFileYn === "Y" ? <Icon>attach_file</Icon> : "-"}
      </MDTypography>
    ),
    regId: (
      <MDTypography variant="caption" color="text" fontWeight="regular">
